...
void createImage(VkImage& image, VkDeviceMemory& imageMemory, const VkDevice& device, const VkPhysicalDevice& physDevice,
...
void createRenderPass(VkRenderPass& renderPass, const VkDevice& device, const VkPhysicalDevice& physDevice,
...
void createCommandPool(VkCommandPool& commandPool, const VkDevice& device, const VkPhysicalDevice& physDevice,
...
void createDescriptorSetLayout(VkDescriptorSetLayout& descriptorSetLayout, const VkDevice& device, bool addSampler) {
...
void createDescriptorPool(VkDescriptorPool& descriptorPool, uint32_t maxFramesInFlight, const VkDevice& device) {
...
void createDescriptorSets(std::vector<VkDescriptorSet>& descriptorSets, uint32_t maxFramesInFlight,
...
void createGraphicsPipeline(VkPipelineLayout& pipelineLayout, VkPipeline& graphicsPipeline,
    ...
    bool depthEnabled) {
    VkShaderModule vertShaderModule = createShaderModule(device, readFile(vertShaderCode));
    VkShaderModule fragShaderModule = createShaderModule(device, readFile(fragShaderCode));
...
void createCommandBuffers(std::vector<VkCommandBuffer>& commandBuffers, uint32_t maxFramesInFlight,
...
void createSyncObjects(std::vector<VkSemaphore>& imgAvailableSmphs, std::vector<VkSemaphore>& renderFinishedSmphs,
    ...
    imgAvailableSmphs.resize(maxFramesInFlight);
    renderFinishedSmphs.resize(maxFramesInFlight);
    ...
    for (size_t i = 0; i < maxFramesInFlight; i++) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imgAvailableSmphs[i]) != VK_SUCCESS ||
            vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSmphs[i]) != VK_SUCCESS ||
